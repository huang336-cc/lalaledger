...
**No login, no ads, no cloud services** — all data stays on your device.
...
| Trip bookkeeping | Trip-book exclusive: **fellow-member management** (add/edit/delete, 10 color tags); when recording, pick **owner (who spent)** and **payer (who paid)** separately, defaulting to self, with quick-add on the record page; bill list / detail / export image show **owner & payer dual capsule tags** (light/dark adaptive); deleting a member keeps historical bills; first-visit guide card |
| Backdated entry | Optional date & time per bill (date picker + 24-hour time picker), defaults to now — perfect for backfilling |
| Quick recording | Large amount card **opens a floating numeric keypad** (overlay style, never covers categories); auto-collapses after picking a category; **swiping the main UI never collapses the keypad — only tapping blank space does** (displacement threshold filters small jitters); expense/income capsule toggle |
...
| Categories | 14 preset expense + 5 income categories, **fully laid out** (no add button); long-press any category to rename / re-icon / re-color |
...
│   │   ├── Migrations.kt      # Migration chain v1→v2→v3→v4 (field/table templates)
│   │   ├── dao/               # Room DAOs (reactive Flow queries)
│   │   └── entity/            # TransactionEntity / CategoryEntity / AccountBookEntity
│   │                          #   / PlaceEntity / MemberEntity (with payerMemberId FK)
│   ├── prefs/SettingsDataStore.kt  # Theme / current book / language / guide-read flags
│   └── repository/            # Repositories: in-memory aggregation + DAO wrappers (SeedData)
├── domain/
...
│   ├── record/                # Record: amount keypad / member dual pickers / date-time / flat categories
│   ├── stats/                 # Stats: pie chart / ranking / member breakdown / AA settlement
│   ├── detail/                # Bill detail
│   ├── books/  members/       # Book management / member management
...
| `members` | `bookId(FK CASCADE) / name / color` | Trip members; deleting a member keeps bills (FK SET_NULL) |
...
- **Keypad collapse gesture**: custom `awaitEachGesture` logic — movement beyond `touchSlop×4` after press, or consumed scroll events → treated as swipe (do not collapse); only "tap on the main UI" collapses.
...
# Release build (R8 + signing, output app/build/outputs/apk/release/lalaledger-v1.4.2.apk)
