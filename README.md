...
### ⚡ 快速记账，一秒一笔
- 大号金额卡点击弹出数字键盘（浮层滑入，不遮挡分类区），选完分类自动收起；**分类紧随金额与类型胶囊**（金额 → 类型 → 分类，第二步即选图标）
...
- **导出弹窗二选一**：统计图片（1080px PNG 存相册）或 **CSV 账单文件**（当前筛选明细，UTF-8 带 BOM，Excel 直接打开，存 Download/拉拉记账）
...
| 统计导出 | 统计页顶栏导出按钮弹出**二选一弹窗**：① 统计图片 —— 1080px PNG 长图（汇总卡 + 分类排行比例条 + 成员明细 + AA 结算方案），深浅主题自适应，存相册「Pictures/拉拉记账」；② CSV 账单 —— 当前筛选后的全部明细（时间/类型/金额/分类/备注/位置/归属/垫付），UTF-8 带 BOM，存「Download/拉拉记账」，导出与导入表头互通 |
| CSV 导入 | 我的页「导入 CSV 账单」选择文件 → 解析预览（笔数、支出/收入、合计、时间范围、将新建的分类与成员、跳过行数）→ 确认后批量写入当前账本；支持中英文表头、多种时间格式、无表头文件按默认列序读取，分类按「类型+名称」匹配、缺失自动新建，成员按名称匹配、缺失自动新建 |
...
| 首页账单列表 | 最近账单**按日期分组**显示，组头展示当日「支出/收入」合计并**可点击折叠**；条目含分类图标、**分类名作标题**、备注以小字列在时间旁、金额，长按直接进入多选 |
...
# Release 构建（R8 + 签名，产物 app/build/outputs/apk/release/lalaledger-v1.7.1.apk）
