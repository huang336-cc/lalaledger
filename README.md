...
拉了记账是为「想搞清楚钱花在哪儿，又不想把流水交给任何云端」的人做的记账工具：
...
- **导出弹窗二选一**：统计图片（1080px PNG 存相册）或 **CSV 账单文件**（当前筛选明细，UTF-8 带 BOM，Excel 直接打开，存 Download/拉了记账）；弹窗内置「**隐藏金额**」开关，打开后导出的图片与 CSV 中所有金额替换为「¥***」（CSV 为 `***`），打码分享不尴尬
...
| 统计导出 | 统计页顶栏导出按钮弹出**二选一弹窗**：① 统计图片 —— 1080px PNG 长图（汇总卡 + 分类排行比例条 + 成员明细 + AA 结算方案），深浅主题自适应，存相册「Pictures/拉了记账」；② CSV 账单 —— 当前筛选后的全部明细（时间/类型/金额/分类/备注/位置/归属/垫付/心情），UTF-8 带 BOM，存「Download/拉了记账」，导出与导入表头互通；弹窗内「**隐藏金额**」开关对两种导出同时生效（全部金额 →「¥***」/`***`，AA 应收应付与转账方案同步打码） |
